#!/usr/bin/env bash
# Tier-1 eBPF beep smoke harness.
#
...
#   2. copies the loader binary + scripts/smoke-remote.sh into
...
#
# Usage: scripts/smoke.sh [--vm <lima-vm-name>]
#
# Host prerequisites (same as README.md's "Building",
# plus cargo-zigbuild for the macOS-host -> Linux-VM cross build this
# harness needs since this crate cannot compile natively on macOS):
...
echo "==> [2/4] cross-building beep-ebpf + beep (nightly + bpf-linker + zigbuild -> aarch64-unknown-linux-gnu)"
